    ...
    main_figs = "\n\n".join(
        [
            ...
            figure_block(
                ...
                "Baseline optimal-policy experiment.  The optimal-policy networks satisfy the static repair projection but generally select the no-repair branch in the reliable checkpoints.  This is treated as a substantive robustness result rather than as evidence against the rule-based mechanism.",
                "fig:optimal-no-repair-d3",
            ),
            figure_block(
                "figures/optimal_policy/optimal_active_region_diagnostic_D_3x.png",
                "Targeted active-region optimal-policy diagnostic.  Even when the state distribution is shifted toward the active-repair reference region, the reliable discretion/commitment checkpoints do not robustly keep repair active.  This is why the paper separates the implementable-rule mechanism from the more fragile full optimal-policy exercise.",
...
def figure_caption(rel: str) -> str:
    ...
    if "all_saved_policy_diagnostic" in rel:
        ...
        return f"All saved policy diagnostics for {tex_escape(scenario)}.  This is a broader checkpoint-level diagnostic used to verify that the main policy-comparison pattern is not driven by a single plotted series."
    ...
    if "local_monetary_wedge" in rel:
        return "Local monetary-wedge diagnostic.  The graph applies a small policy-rate wedge around a repair-active state to isolate the financing channel in the repair KKT condition."
    ...
    if "optimal_policy" in rel:
        return "Optimal-policy diagnostic.  The figure documents whether discretion or commitment converges to a no-repair branch or can sustain an active-repair path under targeted training."
...
def build_appendix_tex(fig_names: list[str]) -> str:
    blocks = [
        ...
        "This appendix collects all figures extracted from the consolidated results archive.  The main text uses a subset of these figures; the remaining figures document checkpoint robustness, sensitivity checks, stochastic steady-state distributions, natural-benchmark decompositions, and local monetary-wedge diagnostics.",
    ]
