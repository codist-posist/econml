...
def _append_optimal_training_args(cmd: list[str], args: argparse.Namespace) -> list[str]:
    ...
    cmd += [
        ...
        "--full-batch-size",
        str(args.optimal_full_batch_size),
        "--private-loss-weight",
...
def main() -> None:
    ...
    parser.add_argument("--fixed-episode-updates-per-episode", type=int, default=2)
    ...
    parser.add_argument("--fixed-rule-scenario-q-weight", type=float, default=25.0)
    ...
    parser.add_argument("--fixed-rule-scenario-loss-interval", type=int, default=25)
