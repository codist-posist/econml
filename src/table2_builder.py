...
def _solve_flex_c_batch(
    ...
    *,
    ...
    eta = regime_eta(params, s.to(torch.long)).to(dtype=dt)
...
def _simulate_flex_prices_for_table2(
    ...
    *,
    ...
    # start near SSS in normal regime (fallback: first available regime)
    ...
    for t in range(int(T + burn_in)):
        ...
        # Regime transition probabilities to all next regimes.
        probs_next = transition_probs_to_next_regimes(params, s.to(torch.long), xi=xi)
        ...
        for sp in range(R):
            ...
            c_sp = _solve_flex_c_batch(
                ...
                xi=xi_n_sp.reshape(-1),
                s=s_sp.reshape(-1),
                h_init=h_init_n,
            ).reshape(B, N)
            lam_next_by_regime.append(c_sp.pow(-params.gamma))

        # E_t[lambda_{t+1}] = sum_{s'} P(s'|s_t) * E[lambda_{t+1}(s') | s']
        Et_lam_next = torch.zeros((B,), device=dev, dtype=dt)
        for sp in range(R):
            e_sp = (lam_next_by_regime[sp] * w_nodes[None, :]).sum(dim=1)
            Et_lam_next = Et_lam_next + probs_next[:, sp] * e_sp
...
def _load_author_flex_regime_moments(
    ...
    *,
    ...
    Flex row from author post-process files:
      - cons_flex_y, i_flex_y from simulated_definitions_*.npz by regime
    ...
    *,
    ...
    Uses:
      ...
      - Source controlled by `sss_source`:
          * "sim_conditional" (paper/author default): regime-conditional
            long-simulation moments (prefer author `simulated_definitions_NT/SS/SEV.npz`,
      ...
      - If `strict_author_table2=True` and `sss_source='sim_conditional'`,
        require author by-regime files (NT/SS/SEV/...) for trained policies (no silent fallback).
    ...
    def add_block(
        ...
        *,
        ...
        if sss_source_norm == "sim_conditional":
            if (policy_key == "flex") and (run_dir is not None):
                ...
                if strict_author_table2 and am_flex is None:
                    fname = _author_regime_filename(int(regime))
                    print(
                        f"[build_table0] WARNING: strict_author_table2=True requires '{fname}' with "
                        f"cons_flex_y/i_flex_y for flex row (run_dir={run_dir}), but file is missing. "
                        "Using fallback moments."
                    )
                ...
                if strict_author_table2 and am is None:
                    fname = _author_regime_filename(int(regime))
                    print(
                        f"[build_table0] WARNING: strict_author_table2=True requires '{fname}' for "
                        f"policy='{policy_key}' in run_dir={run_dir}, but file is missing. "
                        "Using fallback moments."
                    )
                if am is not None:
                    pi_m = am["pi"]
        ...
        rows.append({
            "policy": label,
            "regime": _regime_name(int(regime)),
    ...
    def add_nan_block(label: str, regime: int) -> None:
        rows.append(
            {
                "policy": label,
                "regime": _regime_name(int(regime)),
    ...
    for label, pkey in policies:
        ...
        try:
            ...
            print(f"[build_table0] WARNING: policy='{pkey}' is unavailable; filling table rows with NaN. reason: {e}")

    # Pre-compute regime-conditional policy SSS objects once (used by fixed_point and deterministic modes).
    for label, pkey in policies:
        if pkey is None or pkey not in nets:
            continue
        for s in regime_ids:
    ...
    for cand in (
        ...
        if not rd:
            continue
        files = [_find_author_defs_file(rd, _author_regime_filename(s)) for s in regime_ids]
    ...
    if strict_author_table2 and sss_source_norm == "sim_conditional" and flex_author_run_dir is None:
        print(
            "[build_table0] WARNING: strict_author_table2=True requested, but author by-regime "
            "flex files are missing. Falling back to computed moments where possible."
        )
    ...
    for label, pkey in policies:
        if pkey is None or pkey == "flex" or pkey not in nets:
            continue
        try:
    ...
    for label, pkey in policies:
        ...
        for s in regime_ids:
            ...
            if (sss_source_norm == "sim_conditional") and (sim is None):
                print(
                    f"[build_table0] WARNING: missing sim_paths moments for policy='{pkey}' "
                    "in sim_conditional mode; filling rows with NaN."
                )
                add_nan_block(label, s)
                continue
