...
def transition_probs_to_next_regimes(
    ...
    *,
    ...
    Returns probs with shape s.shape + (2,), where probs[..., j] = Pr(s_{t+1}=j | state_t).
    """
    if int(params.n_regimes) != 2:
        raise NotImplementedError("transition_probs_to_next_regimes currently supports only n_regimes=2")
    ...
    p_to_0 = torch.where(s_t == 0, 1.0 - p12, p21)
    p_to_1 = 1.0 - p_to_0
    return torch.stack([p_to_0, p_to_1], dim=-1)
